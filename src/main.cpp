...
int main() {
	...
	gameLoader.loadStartScreen(window, font);
	...
	SmallPig* s1Pig = memoryPool.getSmall(world, 260.0f, 483.0f, pigTexture);
	...
	MediumPig* s2Pig = memoryPool.getMedium(world, 495.0f, 560.0f, pigTexture);
	...
	LargePig* s3Pig = memoryPool.getLarge(world, 650.0f, 552.0f, pigTexture);
	s3Pig->spawn(&gameObjects);

	Plank s3TopBlock(world, 650.0f, 460.0f, 25.0f, 10.0f, blocksTexture);
	s3TopBlock.spawn(&gameObjects);
	SmallPig* s3TopPig = memoryPool.getSmall(world, 650.0f, 441.0f, pigTexture);
	...
	SmallPig* i1Pig = memoryPool.getSmall(world, 360.0f, 397.0f, pigTexture);
	...
	SmallPig* i2Pig = memoryPool.getSmall(world, 540.0f, 297.0f, pigTexture);
